...
class Whisperer implements ArrayAccess, Arrayable, Jsonable, JsonSerializable
{
    ...
    {
        try {
            return $this->fill(static::$client::find($identifier))->syncOriginal();
        } catch (\Throwable $e) {
            return;
        }
    }
    ...
    {
        if (is_null($model = $this->find($identifier))) {
    ...
    {
        return call_user_func_array([static::$client, $method], [$this->getKey(), $this->toArray()] + $this->$parameters);
    }
